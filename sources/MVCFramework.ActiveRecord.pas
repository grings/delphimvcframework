...
{$ENDIF}
  ...
  public
    ...
    /// <summary>The primary-key value(s) rendered as a single string, columns
    /// joined by ADelimiter (used to build URLs / Location headers).</summary>
    function PKAsDelimitedString(const ADelimiter: string): string;
...
function TMVCActiveRecord.PKAsDelimitedString(const ADelimiter: string): string;
var
  I: Integer;
  lValue: TValue;
  lPart: string;
begin
  ...
  for I := 0 to High(fTableMap.fPrimaryKeys) do
  begin
    lValue := fTableMap.fPrimaryKeys[I].RTTIField.GetValue(Self);
    if lValue.Kind = tkRecord then
    begin
      // Nullable PK column: unwrap the common composite-PK nullable types.
      if lValue.IsType<NullableInt64> then
        lPart := lValue.AsType<NullableInt64>().Value.ToString
      else if lValue.IsType<NullableInt32> then
        lPart := lValue.AsType<NullableInt32>().Value.ToString
      else if lValue.IsType<NullableString> then
        lPart := lValue.AsType<NullableString>().Value
      else if lValue.IsType<NullableTGUID> then
        lPart := lValue.AsType<NullableTGUID>().Value.ToString
      else
        lPart := lValue.ToString;
    end
    else
      lPart := lValue.ToString;
    if I > 0 then
...
begin
  if fTableMap.IsCompositePK then
    raise EMVCActiveRecord.Create(
      'Refresh is not available for composite primary keys; reload with LoadByPKs(...) instead');
